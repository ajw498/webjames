    ...
    if (conn->file) {   // read from file (possibly through temp buffer)
      if (conn->filebuffer) {
        ...
        if (conn->leftinbuffer == 0) {
          conn->leftinbuffer = fread(conn->filebuffer, 1,
                                     readaheadbuffer*1024, conn->file);
          conn->positioninbuffer = 0;
        }
        if (bytes > conn->leftinbuffer)  bytes = conn->leftinbuffer;
        bytes = ip_write(socket, conn->filebuffer + conn->positioninbuffer,
                                                                    bytes);
...
// return filetype or error-code, fill in date (secs since 1990) and mimetype
  ...
  if (xosfile_read_stamped_no_path(filename, &objtype, &load, &exec, size,
                   &attr, &filetype))  return FILE_ERROR;
